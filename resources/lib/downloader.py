...
def download(name, image, url):
    ...
    content = re.findall('(.+?)\s+(\d+)\.\s+\\xc3\\xa9vad\s+(\d+)', name)
    ...
    if not content:
        ...
        control.makeFile(dest)
        transtvshowtitle = content[0][0]
        dest = os.path.join(dest, transtvshowtitle)
        control.makeFile(dest)
        dest = os.path.join(dest, u'\u00C9vad '.encode('utf-8') + '%01d' % int(content[0][1]))
        control.makeFile(dest)
    ...
    if not content:
        ...
        dest = os.path.join(dest, transtvshowtitle + ' S' + content[0][1].zfill(2) + ' E' + content[0][2].zfill(2) + '.' + ext.encode('utf-8'))
    ...
    script = script.decode('iso-8859-1').encode('utf8')
...
def doDownload(url, dest, title, image, headers):
    ...
    if xbmcgui.Dialog().yesno(u'Let\u00F6lt\u00E9s j\u00F3v\u00E1hagy\u00E1sa', file, u'F\u00E1jlm\u00E9ret: %dMB' % mb, u'Tov\u00E1bb a let\u00F6lt\u00E9shez?', 'Igen',  u'M\u00E9gse') == 1:
        return
